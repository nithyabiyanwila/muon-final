...
# Four views: Counts(min), Counts(hourly mean), Deviation%(min vs overall), Deviation%(hour mean vs overall)
...
# ---------------- Time range selector (main page) ----------------
min_t, max_t = work_all["time"].min(), work_all["time"].max()
st.subheader("Time range")
st.caption("Use the slider or type exact timestamps (YYYY-MM-DD HH:MM:SS). Must lie within the file’s range.")
default_range = (min_t.to_pydatetime(), max_t.to_pydatetime())
sel_range = st.slider(
    "Select time window",
...
if pd.notna(typed_start) and pd.notna(typed_end) and (min_t <= typed_start <= typed_end <= max_t):
    start_dt, end_dt = typed_start, typed_end
else:
...
# Status line about clipping
st.caption(
    "2σ clipping applied to counts within the selected window — "
